...
**Submission Instructions:**
  ...
  - The team leader should [create the team repository using GitHub Classrooms](https://classroom.github.com/a/Gqsny7Gh), entering the chosen team name. Make sure that nobody else has already picked your name!  Other team members should then join the team. *It is hard for us to delete teams or move people between them, so be careful and don't join the wrong team! We may deduct points if you cause us extra work.*
...
*Parallel File Uploading*: Implement a system to read huge files (5+ GB) and upload into a Cloud Storage such as AWS S3 and do some processing on them with Serverless services such as AWS Lambda

*Distributed ML system*: Implement a distributed ML model that can be distrabuted on Training model or Training Data Set 
